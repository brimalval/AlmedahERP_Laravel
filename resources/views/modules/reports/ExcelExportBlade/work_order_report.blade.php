        ...
        @if(!empty($table_data))
            @foreach($table_data as $index => $value)
                <tr>
                    ...
                    <td style="border:1px solid black;text-align:center;vertical-align:center; {{$has_width ? 'width:100px' : ''}}">{{date('F d, Y',strtotime($value['planned_start_date']))}}</td>
                    <td style="border:1px solid black;text-align:center;vertical-align:center; {{$has_width ? 'width:100px' : ''}}">{{date('F d, Y' , strtotime($value['planned_end_date']))}}</td>
                    <td style="border:1px solid black;text-align:center;vertical-align:center; {{$has_width ? 'width:100px' : ''}}">{{date('F d, Y' , strtotime($value['real_start_date']))}}</td>
                    <td style="border:1px solid black;text-align:center;vertical-align:center; {{$has_width ? 'width:100px' : ''}}">{{date('F d, Y' , strtotime($value['real_end_date']))}}</td>
                    <td style="border:1px solid black;text-align:center;vertical-align:center; {{$has_width ? 'width:100px' : ''}}">{{$value['work_order_status']}}</td>
                </tr>
            @endforeach
        @endif
    </tbody>
